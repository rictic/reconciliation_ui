...
function displayReconChoices(entityID:string) {
  ...
  if (!$(".displayArea " + selector)[0]) {
    var entityScreen = $("#" + getManualReconId(entity));
    $(".displayArea").empty().append(entityScreen)
    entityScreen.find('.find_topic').focus();
  }
...
function renderReconChoices(entity:tEntity) {
  ...
  $(".search-row td", template).attr('colspan', 1 + columnHeaders.length)
  ...
  } else if (entity.reconResults.length == 0) {
    $('.onlyWhenCandidates', template).hide();
  } else if (entity.reconResults.length < 4) {
    $(".moreButton", template).hide();
  }
